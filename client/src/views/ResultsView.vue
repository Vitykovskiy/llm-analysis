...
import { ref } from 'vue'
import { getSimilarMessages, isDemoMode, type SimilarEntry } from '../services/api'
...
const fetchResults = async (): Promise<void> => {
  ...
  error.value = ''
  try {
    results.value = await getSimilarMessages(payload, limit.value || 3)
  } catch (err) {
        ...
        <v-card elevation="3">
          <v-card-title class="d-flex align-center justify-space-between flex-wrap ga-4">
            ...
            <div class="d-flex ga-2">
              <v-text-field v-model="query" label="Запрос" density="comfortable" variant="outlined" hide-details
                class="results-query" @keydown.enter.prevent="fetchResults" />
              <v-text-field v-model.number="limit" label="Лимит" type="number" min="1" max="10" density="comfortable"
                variant="outlined" hide-details style="max-width: 120px" @keydown.enter.prevent="fetchResults" />
              <v-btn color="primary" :disabled="!query.trim() || loading" :loading="loading" prepend-icon="mdi-magnify"
                @click="fetchResults">
                Искать
              </v-btn>
            </div>
          ...
          <v-card-text class="d-flex flex-column ga-4">
            ...
            <div v-else class="d-flex flex-column ga-3">
              ...
              <v-card v-for="(item, index) in results" :key="index" variant="outlined" class="pa-3">
